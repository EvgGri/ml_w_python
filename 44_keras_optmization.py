...
# Мы можем еще попробовать изменить скорость обучения опти- мизатора. Из следующего графика видно, что оптимальное значение близко к 0.001,
# а это как раз и есть значение, подразумеваемое по умолчанию. Прекрасно! Adam работает, не требуя никакой настройки.

# -=-=-=-=-=- Увеличение числа нейронов в скрытых слоях
# Еще одна возможность – изменить число нейронов во внутренних скрытых слоях. На следующем графике показаны результаты, получаемые при увеличении числа нейронов,
# т. е. сложности модели. Как видно, время вычислений быстро растет, поскольку приходится оптимизировать все больше параметров.
# Но достигаемый выигрыш при этом становится все меньше и меньше.

# -=-=-=-=-=- Увеличение размера пакета
# Алгоритм градиентного спуска пытается минимизировать функцию стоимости одновременно на всех примерах из обучающего набора и для всех представленных в нем признаков.
# Алгоритм стохастического градиентного спуска обходится гораздо дешевле, потому что в нем рассматривается только BATCH_SIZE примеров.
# Посмотрим, как зависит поведение модели от этого параметра. Как видим, оптимальная верность достигается, когда BATCH_SIZE=128.
